...
A análise exploratória detalhada está disponível no notebook:

- [EDA F1 — Jupyter Notebook](notebooks/eda_f1_vfinal.ipynb)

O notebook apresenta as análises, tratamentos, visualizações e principais
insights obtidos a partir dos dados da camada Silver.

### Streamlit

As análises também estão disponibilizadas por meio de uma aplicação
interativa em Streamlit, permitindo explorar os dados e visualizações de
forma mais dinâmica.

- [Aplicação Streamlit](streamlit/app.py)
...
A etapa de ML contempla a construção e validação do dataset, EDA específica, preparação das features, separação entre desenvolvimento e teste, treinamento, avaliação, tuning e interpretação dos modelos.
...
### 20. Executar a aplicação Streamlit(Somente para a Etapa de Machine Learning e considerações Gabriel Bortoletto)

Com o MinIO em execução e os dados disponíveis na camada Machine Learning:
```
streamlit run streamlit/app.py
```
